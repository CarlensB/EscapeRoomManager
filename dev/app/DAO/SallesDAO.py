# ===============================================
# Nom du fichier : SallesDAO.py
# Ce fichier contient les actions pour interagir
# avec la table contenant les salles.
...
class SallesDAO:
    ...
    def ajouter(self, args : list[tuple[str, str, int, int, float, int]]) -> None:
        ...
        sql = "SELECT id FROM salles WHERE nom = %s AND description = %s AND centre = %s AND nb_max_joueur = %s AND prix_unitaire = %s AND privee = %s"
